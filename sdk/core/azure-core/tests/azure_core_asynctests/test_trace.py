...
class MockClient:
    ...
    async def __init__(self, policies=None, assert_current_span=False):
...
class TestUseDistributedTraces(unittest.TestCase):
    async def test_use_distributed_traces_decorator(self):
    ...
    async def test_parent_span_with_opencensus(self):
    ...
    async def get_children_of_datadog_span(self, parent, tracer):
    ...
    async def test_with_parent_span_with_datadog(self):
        client = MockClient(policies=[DistributedTracer()])
    ...
    async def test_trace_with_not_setup(self):
    ...
    async def test_blacklist_works(self):
        trace = tracer.Tracer(sampler=AlwaysOnSampler())
        parent = trace.start_span(name="OverAll")
        client = MockClient(policies=[DistributedTracer()])
        await client.make_request(2, tracer="opencensus")
    ...
    async def test_without_parent_span_with_tracing_policies(self):
        client = MockClient(policies=[DistributedTracer()])
        res = await client.make_request(2)
        assert res is client.expected_response

    async def test_with_parent_span_without_tracing_policies(self):
        client = MockClient(policies=[])
        parent = Span(name="Overall")
    ...
    async def test_without_parent_span_without_tracing_policies(self):
        client = MockClient(policies=[])
        res = await client.make_request(2)
        assert res is client.expected_response
